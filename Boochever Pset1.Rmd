...
\textcolor{blue}{\textit{Some of the variables in the second regression are explainable by or have correlation with variables in the first regression. That variables may also have correlation with the dependent variable. In essence, there is omitted variable bias, and the coefficients in the second model either overstate or understate their true predictive power compared to when controlling for relevant additional confounders.}}
...
\textcolor{blue}{\textit{The longer "kitchen sink" linear regression performed better with a lower MSE than the either the better performing KNN(10) model. }}
\
